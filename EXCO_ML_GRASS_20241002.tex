...
\subsection{\textcolor{blue}{State-of-the-art}}
In the early 1970s, when the GIS\hl{ }has started its development, the cartographic possibilities \hl{of satellite image processing }were limited to the existing restricted functionality of GIS\hl{. In contrast,} modern \hl{frameworks use} programming libraries to enhance technical capabilities of GIS through the AI and ML. To achieve the main objective of RS data processing, that is, to accurately and rapidly extract information from satellite images, \hl{the} data should be processed using advanced computer-based techniques. In this case, the algorithms of RS data processing leverage diverse classification methods \cite{su151813786,Lemenkova202229}. 
...
At the next step, 10 groups of land cover categories were defined using information received on previous step in order to recognise these classes on actual study area. Following that, the images were processed using four different ML algorithms of GRASS GIS by the combination of modules "r.random" (which generates training pixels from the land cover classification), "r.learn.train" (which trains models using various ML algorithms) and "r.learn.predict", which performs \hl{the} prediction of the pixels' assignment into target categories based on the \hl{values of their} spectral signatures. 

\hl{The accuracy of the assigned classes was estimated using Kappa statistics which evaluates chance agreement of the correctly classified pixels. This method is based on the estimation of the observed versus expected values of pixels in each land cover class of the output raster matrix. Practically, it has been calculated module "r.kappa" of GRASS GIS to validate the results of image classification. The training dataset was based on the raster data from the Global Land Cover Characteristics (GLCC) achieved from the National Oceanic and Atmospheric Administration (NOAA) Advanced Very High Resolution Radiometer (AVHRR) Pathfinder satellite images.}
...
\hl{The correct identification of the land cover classes is important because the results of mapping can be used for predicting agricultural productivity, land use planning and management. Therefore, it is essential to evaluate the accuracy of image classification using ML algorithms. In GRASS GIS, the Estimated kappa ($\kappa$) value in "r.kappa" is the value that was computed for land cover classes, that is, each of the 10 categories. Hence, the observed values of the classified images were evaluated against the training polygons for land cover categories into the target class. The GLCC data were used as reference to validate the results of the classified images. }

\hl{A sample of points as a ground truth data from the GLCC was compared to the classification results to determine the accuracy of the evaluation. Methodologically, the estimation of kappa ($\kappa$) presents the observed agreement between the results of the assigned pixels for those cells that have been classified by each classifier (RF, DT, MLPC and SVM) into the target class of land cover categories. In GRASS GIS, kappa statistics was calculated using "r.kappa" following the formula in Equation} \ref{eq7}: 

\begin{equation}
kpp_i = (pii_i - pi_i * pj_i) / (pi_i - pi_i * pj_i); 
\label{eq7}
\end{equation}

\hl{where the $pii_i$ is the number of pixels for which there is agreement divided by overall number of the classified pixels which expresses the probability of agreement; $Pi_i$ is the probability of the correct classification $i$ where the target pixel is assigned to the correct category of land cover class $i$; $Pj_i$ is the probability of classification $j$ to accurately classify pixel $i$.} \hl{Practically, the validation was performed using GRASS GIS syntax where input data is given with an example for 2023 and reference taken from t he GLCC file. Afterwards, the results on $\kappa$ statistics were exported as csv file. In GRASS GIS, the accuracy assessment was computed using the following code:}

\begin{footnotesize}
\begin{verbatim}
g.region raster=L8_2023_Faiyum -p 
r.kappa -w classification=L8_2023_Faiyum \
	reference=GLCC_Egypt output=kappa.csv -m -h
# saving Kappa matrix to tabular format
r.kappa classification=L8_2023_Faiyum \
	reference=GLCC_Egypt output=k_Faiyum.csv -m -h
\end{verbatim}
\end{footnotesize}
...
The assessment of the statistical analysis\hl{ }for the year of 2013 \hl{shows} the final results report convergence=98.3\% and 98.26\% points stable. For the year 2023, the classification results report 98.01\% points stable and convergence=98.0\%. Since the computed accuracy is higher than 80\%, the classification of the multispectral image is performed robustly. Therefore, the application of the ML, as demonstrated in this study, can be advised for similar future studies where image processing is applied for environmental monitoring and mapping. \hl{With a kappa value ($\kappa$) of 0.74, the study's total classification accuracy was 82.4\% for image on year 2013 and kappa value ($\kappa$) of 0.73 and classification accuracy was 84.2\% for image on 2023. Since the $\kappa$ coefficient is considered significant for both images (on 2013 and 2023), it points at the applicability of the ML methods of GRASS GIS and their effectiveness for investigations on image processing in future similar research. }
