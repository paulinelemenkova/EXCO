...
The images were selected based on their quality, coverage and suitability, and downloaded from the EarthExplorer website: \href{https://earthexplorer.usgs.gov/}{\hl{https://earthexplorer.usgs.gov/}}. 
...
Among the compared outputs, the MLP and SVC Classifiers differ from the \hl{RF} and \hl{NB} approaches, due to the different approaches (ANN and SVM methods). The map generated using Naive Bayes Classified (Figure \ref{fig03}, a) is comparable with the map based on the \hl{RF} Classifier (Figure \ref{fig03}, d), while MLP Classifier presented a more generalised map (Figure \ref{fig03}, b) using deep learning network approach. 
