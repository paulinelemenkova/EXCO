...
The \hl{SV Classifier} is a branch of the \hl{SVM} which are a set of supervised learning methods \cite{Cortes}. \hl{Based on the statistical learning frameworks developed by} \cite{Vapnik}, \hl{SVM is now among the most applicable models due to the efficiency of its algorithm}. The main approach of the SVM is classifying the new data into one or two given classes using the decision process which tracks the largest separation, i.e., margin, between the given classes. Hence, the a best separation is obtain using the discrimination of the location of the samples within the finite-dimensional space of dataset, and detected largest distance to the nearest training sample of one of these classes \cite{6069282,earth5030024}.
