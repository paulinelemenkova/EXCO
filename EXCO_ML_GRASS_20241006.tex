...
\subsection{Data}
The data are presented by the Landsat 8 OLI-TIRS satellite images covering western segment of the Nile Delta and the Faiyum Oasis, upper Egypt in 2013 and 2023, Figure \ref{fig01}. The images were selected based on their quality, coverage and suitability, and downloaded from the EarthExplorer website. 
...
\begin{table*}[t]
    ...
    \end{tabular}
    \caption{Technical parameters of the Landsat-8 OLI/TIRS images used as dataset. Source: EarthExplorer by the United States Geological Survey (USGS).}
    }
    \label{tab01}
\end{table*}

The individual technical characteristics of the two images are summarised in Table \ref{tab01}. The data are provided by the USGS as open source repository from the Collection Number 2, Category T1. The row/path of the Landsat swath is 39/177 for both the scenes, collected in dat time in nadir. Both images were projected in the Universal Transverse Mercator (UTM) Zone 36 for Egypt in WGS84 Datum and Ellipsoid, Station Identifier -- LGN; the Ground Control Points Version used for data correction is 5. The images on 2013 and 2023 were selected to evaluate the decade dynamics of the landscapes around the Faiyum Oasis, Upper Egypt within the 10-year time span. 
...
where the parameters $\sigma_y$ and $\mu_y$  are computed using the approach of maximum likelihood. Thus, the algorithm of GaussianNB Classifier computes the likelihood of the features using the Bayes' theorem which assumes the independence between every pair of pixels according to Gaussian distribution and considers the value of cells assigned to each class of the classified satellite image as variables. In the scripting environment of GRASS GIS, the Gaussian NB Classifier was implemented using the following script:
...
The mathematical foundation of the Support Vector Machine algorithm consists in the following. When SVC uses training vectors, that is, the sample pixels that are located within the margin of classes,
